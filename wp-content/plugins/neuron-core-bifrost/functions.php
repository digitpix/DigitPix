...
function neuron_core_styles() {
    ...
    if (neuron_custom_fonts_styles()) {
        wp_add_inline_style('neuron-style', implode(' ', neuron_custom_fonts_styles()));
    }
}
...
function neuron_core_scripts() {
    ...
    wp_enqueue_script('justified-gallery', plugin_dir_url(__FILE__) . 'assets/scripts/jquery.justifiedGallery.min.js', array('jquery'), NEURON_CORE_VERSION, TRUE); 
    ...
    // wp_enqueue_script('easings', plugin_dir_url(__FILE__) . 'assets/scripts/jquery.easings.min.js', array('jquery'), NEURON_CORE_VERSION, TRUE);
    // wp_enqueue_script('multiscroll', plugin_dir_url(__FILE__) . 'assets/scripts/jquery.multiscroll.min.js', array('jquery'), NEURON_CORE_VERSION, TRUE);

    if (get_theme_mod('google_maps_api_key')) {
        wp_register_script('neuron-google-maps', 'https://maps.googleapis.com/maps/api/js?key='. get_theme_mod('google_maps_api_key') .'', array('jquery'), NEURON_CORE_VERSION);
    }
}
...
/**
 * External Image Calculation
 * 
 * Calculate external image padding
 */
function neuron_external_image_calculation($width, $height) {
    if (!empty($width) && !empty($height)) {
        return 'padding-bottom: '. number_format($height / $width * 100, 6) .'% !important;';
    }
}
 ...
 */
function neuron_get_elementor_templates($type, $noType = false) {
    $choices = [];
 ...
 * Preview
 */
function neuron_prevent_preview() {
    if (is_preview()) {
        ?>
        <script>
...
add_action('wp_head', 'neuron_prevent_preview');

/**
 * Custom Fonts
 */
function neuron_custom_fonts(){
    if (!get_theme_mod('custom_fonts')) {
        return;
    }
    
    $custom_fonts = [];
    $custom_fonts_options = get_theme_mod('custom_fonts');

    foreach ($custom_fonts_options as $font) {
...
if (get_theme_mod('custom_fonts')) {
    add_filter('kirki/fonts/standard_fonts', 'neuron_custom_fonts', 20);
}

/**
 * Custom Fonts Media
 */
function neuron_custom_fonts_styles() {
    if (!get_theme_mod('custom_fonts')) {
        return;
    }
    ...
    return $neuron_inline_style;
}

/**
 * Placeholder
 */
function neuron_placeholder() {
    echo '<img src=' . esc_url(NEURON_THEME_PLACEHOLDER) . ' alt='. esc_attr__('Placeholder Image', 'neuron-core') . '/>';
}
...
function get_public_post_types( $args = [] ) {
    ...
    return apply_filters( 'bifrost/utils/get_public_post_types', $post_types );
}